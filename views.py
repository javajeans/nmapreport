...
def details(request, address):
				...
				if p['state']['@state'] == 'open':
					if 'service' in p:
						if '@version' in p['service']:
							...
							v = '<i class="grey-text">没有软件信息</i>'

						if '@product' in p['service']:
							z = p['service']['@product']
						else:
							z = '<i class="grey-text">没有版本信息</i>'
	...
	'	$("#detailspo").html(\'<center><h4><i class="fas fa-door-open green-text"></i> '+str(po)+'</h4><span class="small grey-text">开启的端口</span></center>\');'+\
	'	$("#detailspc").html(\'<center><h4><i class="fas fa-door-closed red-text"></i> '+str(pc)+'</h4><span class="small grey-text">关闭的端口</span></center>\');'+\
	'	$("#detailspf").html(\'<center><h4><i class="fas fa-filter grey-text"></i> '+str(pf)+'</h4><span class="small grey-text">过滤的端口</span></center>\');'+\
	'}); '+\
	'</script>'
...
def index(request, filterservice="", filterportid=""):
	...
	if filterservice == "" and filterportid == "":
		...
		'			type: "doughnut", data: {labels:["打开的", "过滤的", "关闭的"], datasets: [{ data: ['+str(ports['open'])+','+str(ports['filtered'])+','+str(ports['closed'])+'], backgroundColor:["rgba(0,150,0,0.8)","rgba(255,200,0,0.8)","rgba(255,0,0,0.8)"], borderColor:"#ccc", borderWidth:0 }]}, options: {legend: { position: "right", labels: { fontColor: "#ccc" }  }}'+\
		'		});'+\
